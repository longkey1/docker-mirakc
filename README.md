...
| コンポーネント | バージョン | 説明 |
|--------------|----------|------|
| [mirakc](https://github.com/mirakc/mirakc) | 3.4.77 | Mirakurun 互換チューナーサーバー |
...
`<tag>` は3つのコンポーネントのバージョンを並べたリリースタグ（例: `3.4.77-1.2.4-0.1.2`）です。各バージョンの先頭の `v` は除去します。
...
タグのフォーマット例: `3.4.77-1.2.4-0.1.2`（`<mirakc>-<recisdb-rs>-<miraview>`、各バージョンの先頭の `v` を除去）
